...
point_t KDTree::nearest_point(const point_t &pt,const point_t &pt2) {
    return point_t(*nearest_(pt,pt2));
};
...
pointIndex KDTree::nearest_pointIndex(const point_t &pt,const point_t &pt2) {
    KDNodePtr Nearest = nearest_(pt,pt2);
...
//    d = disth(point_t(*branch), pt, rad);
    int nodeval=point_t(*branch).at(level);
    int pointval=pt.at(level);
    dx = abs(point_t(*branch).at(level) - pt.at(level));
//    dx2 = dx * dx;

    KDNodePtr section = branch->left;
    KDNodePtr other = branch->right;

    pointIndexArr nbh, nbh_s, nbh_o;
    if (dx <= rad) {
//        std::cout<<dx<<" "<<nodeval<<" "<<pointval<<" "<<rad<<" "<<level<<"\n";
        if(disth(point_t(*branch), pt, rad)>0)
